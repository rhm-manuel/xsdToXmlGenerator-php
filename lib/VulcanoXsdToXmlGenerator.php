...
class VulcanoXsdToXmlGenerator {
	...
	/**
	 * Each node should exist only once.
	 * This was caused by a bug and will maybe fix in the future.
	 * This only affects the parsing of the xsd file.
	 *
	 * @var array
	 */
	private $storedNodes = array();
			...
			if (!empty($this->nodeDefinitionAttributes[$rootNodeName])) {
				foreach( $this->nodeDefinitionAttributes[$rootNodeName] as $keyName => $attr ) {
					...
					elseif( true === $this->isOptionIsEnabled( $option ) ) {
						$message = sprintf(
							'Missing assigned value for attribute: "%s" on node: "%s". Assign is required!',
							$rootNodeName,
							$keyName
						);
	...
	private function arrayValueTamper( &$structure, $key, array $value ) {
		if( isset( $structure[$key] ) ) {
			$structure[$key] = $this->vulcanoMergeArrayRecursive( $structure[$key], $value );
		}
		else {
	...
	/**
	 * Helper function to merge arrays recursive.
	 * The php-function array_merge_recursive has a strange behaviour.
	 *
	 * @return array
	 */
	private function vulcanoMergeArrayRecursive() {
		...
		foreach( $arrays as $array ) {
			...
			while( list( $key, $value ) = @each( $array ) ) {
				if( is_array( $value ) && @is_array( $base[$key] ) ) {
					$base[$key] = $this->vulcanoMergeArrayRecursive( $base[$key], $value );
				}
				else {
					$base[$key] = $value;
				}
